...
export default function Modifyv2() {
    useEagerConnect()
    ...
    const modify = async () => {
        ...
        try {
            const tx = await contract.setLicenseForBASTARD("2",bastardID,text)
    ...
    return (
        <div className="bg-red-500 flex flex-col items-center justify-center p-5">
            <input className={"p-5"} type={"number"} placeholder={"v2 bastard ID"} min={0} max={11305} onChange={e => setBastardID(prev => e.target.value)}/>
